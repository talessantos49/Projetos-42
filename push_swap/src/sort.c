/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   sort.c                                             :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: tasantos <tasantos@student.42sp.org.br>    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/03/20 15:41:44 by marvin            #+#    #+#             */
/*   Updated: 2023/04/11 03:47:42 by tasantos         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/push_swap.h"

void sort_two (t_stack *stack)
{
    if (stack->first->data > stack->first->next->data)
        swap_a(stack);
}

void sort_three (t_stack *stacka)
{
    int first;
    int next;
    int last;

    first = stacka->first->data;
    next = stacka->first->next->data;
    last = last_number(stacka)->data;
    if (next > first && last < next && last < first)
        shift_down_rra(stacka);
    if (last > next && next > first)
    {
        shift_up_ra(stacka);
        swap_a(stacka);
    }
    if (last > next && last > first && first > next)
        swap_a(stacka);
    if (next > last && last > first)
    {
        swap_a(stacka);
        shift_up_ra(stacka);
    }
    if (first > last && last > next)
        shift_up_ra(stacka);
    if (first > next && next > last)
    {
        swap_a(stacka);
        shift_down_rra(stacka);
    }
}

void push_three(t_stack *stacka, t_stack *stackb)
{
    push_b(stacka, stackb);
    sort_three(stacka);
    push_a(stacka, stackb);
}

void sort_four(t_stack *stacka, t_stack *stackb)
{
	t_node	*aux;
	int		min;

	min = stacka->first->data;
	aux = stacka->first;
	while (aux->next)
	{
		if (min > aux->next->data)
			min = aux->next->data;
		aux = aux->next;
	}
	while (stacka->first->data != min)
		shift_up_ra(stacka);
    push_three(stacka, stackb);
}

/* void sort_four(t_stack *stacka, t_stack *stackb)
{
    int first_a;
    int next_a;
    int next_a2;
    int last_a;
	t_node *aux;

    first_a = stacka->first->data;
    next_a = stacka->first->next->data;
    next_a2 = stacka->first->next->next->data;
	aux = last_number(stacka);
	last_a = aux->data;
    if (first_a < next_a && first_a < next_a2 && first_a < last_a)
        push_three(stacka, stackb);
    else if (last_a < first_a && first_a < next_a && next_a < next_a2)
        shift_down_rra(stacka);
    else if (last_a < first_a && last_a < next_a && last_a < next_a2)
    {
        shift_down_rra(stacka);
        push_three(stacka, stackb);
    }
    else if (next_a < first_a && first_a < next_a2 && first_a < last_a && next_a2 < last_a)
		swap_a(stacka);
    else if (next_a < first_a && next_a < last_a && next_a < next_a2)
    {
        swap_a(stacka);
        push_three(stacka, stackb);
    }
    else if (next_a2 < last_a && last_a < first_a && first_a < next_a)
    {
        shift_down_rra(stacka);
        shift_down_rra(stacka);
    }
    else if (next_a2 < first_a && next_a2 < last_a && next_a2 < next_a)
    {
        shift_down_rra(stacka);
        shift_down_rra(stacka);
        push_three(stacka, stackb);
    }
} */
